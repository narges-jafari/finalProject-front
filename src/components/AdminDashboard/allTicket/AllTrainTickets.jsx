...
const AllTrainTickets = (props) => {
    ...
    return (
      <>
       ...
       <div className={styles.headerAllTicket}> 
        بلیط‌های قطار خریداری شده در تاریخ {date}
        ...
        <Table className='w-75  mx-auto'>
          ...
          {data.map((item,index)=>{
            ...
            <tr key={index} className='shadow p-3 mb-5 bg-white rounded' style={{ backgroundColor: 'white' }}>
              ...
              <td className='border-2 border-light ' >{item.trainBuy.train.originName}</td>
              <td className='border-2 border-light ' >{item.trainBuy.train.destinationName}</td>
              <td className='border-2 border-light ' >{item.trainBuy.user.username}</td>
  ...
  export default AllTrainTickets
