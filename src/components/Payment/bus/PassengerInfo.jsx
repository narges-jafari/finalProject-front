...
const PassengerInfo = () => {
  ...
  useQuery(busQueries.SEARCH, {
    ...
    onCompleted: (res) => {
      setPrice(res.searchBusbyId.price)
    },
    onError: () => {
      setPrice([])
    }
  ...
  const handleCreateBuyBus = (e) => {
    if (clicked == true) {
      ...
      createBuyBus({
        ...
        .then(({ data }) => {
          if (data.busBuy !== null) {
            toast.success('بلیط اتوبوس با موفقیت خریداری شد')
            window.localStorage.setItem(BUSBUY_ID, JSON.stringify(data.busBuy._id))
            // window.localStorage.setItem(HOTEL_CAPACITY, JSON.stringify(data.createHotel.capacity))
            window.location.href = '/finishbus'
